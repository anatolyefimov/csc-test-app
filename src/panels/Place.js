...
const Place = ({ item, order, onIncrementPosition, onDecrementPosition, area }) => {
  const total = useMemo(() => {
    ...
    const result = Object.values(order)
      ...
      .reduce((result, value) => {
        ...
        result.price += parseInt(price) * parseInt(count);

        return result;
      }, {count: 0, price: 0});

    return {
      price: accounting.formatNumber(result.price, 0, ' '),
      count: result.count
    }
  }, [ order, item ]);
  ...
  const handleOrder = (event) => {
    if (total.count === 0) {
      ...
      console.error('Нужно выбрать хотя бы один товар!')
    }
  }
